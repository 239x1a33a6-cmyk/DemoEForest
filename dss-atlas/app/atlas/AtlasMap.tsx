      ...
      <Map
        ...
        onClick={(e) => {
          ...
          if (features && features.length > 0) {
            // Create a synthetic event or just pass the necessary data
            // Since onStateClick expects MapLayerMouseEvent, we can just call it with the current event
            // but we need to attach the features we found.
            // However, MapLayerMouseEvent is read-only for features usually.
            // Let's just call onStateClick with the event and let it find features or pass them if we modify onStateClick.
            // Actually, onStateClick uses event.features?.[0].
            // We can just call onStateClick(e) but we need to make sure e has the features.
            // The 'e' from onClick is already a MapLayerMouseEvent.
            // But queryRenderedFeatures returns features that might not be in e.features if we didn't click exactly on it?
            // Wait, interactiveLayerIds handles the click detection for us.
            // So e.features should already be populated if we clicked on an interactive layer.

            // If we are manually querying, we might need to override.
            // But 'highlighted-state-fill' IS in interactiveLayerIds.
            // So e.features should be populated.
