...
export default function AssetDetection() {
  ...
  const generateDetectionResults = (assetData: any, locationData: any) => {
    ...
    if (assetData.forest) {
      ...
      results.push({
        ...
        status: forestArea > 1000 ? 'Dense Coverage' : forestArea > 500 ? 'Moderate Coverage' : 'Sparse Coverage',
        icon: 'ri-tree-line',
    ...
    if (assetData.water) {
      results.push({
        ...
        status: `${assetData.water.sources} Sources Detected`,
        icon: 'ri-drop-line',
        color: 'text-blue-600',
        bgColor: 'bg-blue-50',
        details: {
    ...
    if (assetData.agriculture) {
      ...
      results.push({
        ...
        status: `${assetData.agriculture.productivity} Productivity`,
        icon: 'ri-plant-line',
    ...
    if (assetData.settlement) {
      results.push({
        ...
        status: `${assetData.settlement.population} Population`,
        icon: 'ri-home-4-line',
    ...
    if (assetData.infrastructure) {
      results.push({
        ...
        status: `${assetData.infrastructure.roads} Roads`,
    ...
    if (assetData.minerals) {
      results.push({
        ...
        status: `${assetData.minerals.reserves} Reserves`,
  ...
  }, [selectedLocation, detectionResults.length]);
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg border border-gray-200 p-6">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-lg font-semibold text-gray-900">AI Asset Detection</h3>
        {selectedLocation && (
        ...
        <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <div className="text-sm text-blue-700">Processing satellite imagery for {selectedLocation?.village}...</div>
            </div>
          </div>
      ...
      <div className="space-y-4">
        {detectionResults.map((result, index) => (
          <div 
            key={index} 
            className={`p-4 rounded-lg border ${result.bgColor} border-opacity-50 transition-all duration-300 cursor-pointer hover:shadow-md ${
          ...
          >
            ...
            {!isAnalyzing && (
              <div className="flex justify-center mt-3">
                <div className={`w-6 h-6 flex items-center justify-center ${result.color} transition-transform duration-200 ${
                  expandedCard === result.id ? 'rotate-180' : ''
                }`}>
        ...
        <div className="mt-6 pt-4 border-t border-gray-200">
          <div className="flex items-center justify-between text-sm">
            ...
            <div className="text-gray-500">
              {detectionResults.length} asset types detected
            </div>
          </div>
          
          {selectedLocation && (
            <div className="mt-3 p-3 bg-gray-50 rounded-lg">
              <div className="text-sm text-gray-600 mb-1">Analysis Summary for {selectedLocation.village}</div>
              <div className="text-xs text-gray-500">
                Average Confidence: {Math.round(detectionResults.reduce((acc, r) => acc + r.confidence, 0) / detectionResults.length)}% • 
                Last Updated: {new Date().toLocaleTimeString()} • 
                Data Source: Real-time Satellite Analysis
              </div>
            </div>
          )}
        </div>
            ...
            <div className="p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div className={`p-4 ${selectedAsset.bgColor} rounded-lg border`}>
                  <div className={`${selectedAsset.color} font-medium`}>Area Coverage</div>
                  <div className="text-2xl font-bold text-gray-900">{selectedAsset.area}</div>
                </div>
                <div className={`p-4 ${selectedAsset.bgColor} rounded-lg border`}>
                  <div className={`${selectedAsset.color} font-medium`}>Detection Confidence</div>
                  <div className="text-2xl font-bold text-gray-900">{selectedAsset.confidence}%</div>
                </div>
                <div className={`p-4 ${selectedAsset.bgColor} rounded-lg border`}>
                  <div className={`${selectedAsset.color} font-medium`}>Status</div>
              ...
              <div className="space-y-6">
                {selectedAsset.extendedInfo.species && (
                  <div>
                    <h4 className="font-semibold text-gray-900 mb-2">Species & Biodiversity</h4>
                ...
                {selectedAsset.extendedInfo.waterTypes && (
                  <div>
                    <h4 className="font-semibold text-gray-900 mb-2">Water Source Types</h4>
                    <div className="flex flex-wrap gap-2">
                ...
                {selectedAsset.extendedInfo.cropTypes && (
                  <div>
                    <h4 className="font-semibold text-gray-900 mb-2">Crop Types</h4>
                    <div className="flex flex-wrap gap-2">
                ...
                {selectedAsset.extendedInfo.conservation && (
                  <div>
                    <h4 className="font-semibold text-gray-900 mb-2">Conservation Measures</h4>
                    <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    {selectedAsset.extendedInfo.economicValue && (
                      <div className="text-center p-4 bg-blue-50 rounded-lg">
                        ...
                        <div className="text-sm text-blue-600">Economic Value</div>
                      </div>
                    )}
                    {selectedAsset.extendedInfo.beneficiaries && (
                      <div className="text-center p-4 bg-orange-50 rounded-lg">
                        ...
                        <div className="text-sm text-orange-600">Beneficiaries</div>
                      </div>
                    )}
                  </div>
                </div>
