...
async function processPDF(file: File, onProgress?: (step: string, progress: number) => void): Promise<string> {
    ...
    // pdfjs expects a RenderParameters object that includes the canvas element in newer typings
    await page.render({ canvas: context.canvas as HTMLCanvasElement, canvasContext: context, viewport } as any).promise;
